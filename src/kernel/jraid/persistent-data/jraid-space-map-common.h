...
#ifndef __JD_SPACE_MAP_COMMON_H__
#define __JD_SPACE_MAP_COMMON_H__
...
typedef int (*load_ie_fn)(struct ll_disk *ll, jd_block_t index, struct disk_index_entry *result);
typedef int (*save_ie_fn)(struct ll_disk *ll, jd_block_t index, struct disk_index_entry *ie);
typedef int (*init_index_fn)(struct ll_disk *ll);
typedef int (*open_index_fn)(struct ll_disk *ll);
typedef jd_block_t (*max_index_entries_fn)(struct ll_disk *ll);
typedef int (*commit_fn)(struct ll_disk *ll);

struct ll_disk {
	struct jd_transaction_manager *tm;
	struct jd_btree_info bitmap_info;
	struct jd_btree_info ref_count_info;

	uint32_t block_size;
	uint32_t entries_per_block;
	jd_block_t nr_blocks;
	jd_block_t nr_allocated;

	/*
	 * bitmap_root may be a btree root or a simple index.
	 */
	jd_block_t bitmap_root;

	jd_block_t ref_count_root;
...
int sm_ll_insert(struct ll_disk *ll, jd_block_t b, uint32_t ref_count, enum allocation_event *ev);

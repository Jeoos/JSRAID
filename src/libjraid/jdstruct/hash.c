...
static struct jd_hash_node *_create_node(const char *str, unsigned len)
{
	struct jd_hash_node *n = malloc(sizeof(*n) + len);
...
struct jd_hash_table *jd_hash_create(unsigned size_hint)
{
	...
	struct jd_hash_table *hc = malloc(sizeof(*hc));
...
static void _free_nodes(struct jd_hash_table *t)
{
	struct jd_hash_node *c, *n;
	unsigned i;
...
void *jd_hash_lookup_binary(struct jd_hash_table *t, const void *key,
			    uint32_t len)
{
	struct jd_hash_node **c = _find(t, key, len);
			  ...
			  uint32_t len, void *data)
{
	...
	if (*c)
		...
		struct jd_hash_node *n = _create_node(key, len);
...
{
	return jd_hash_insert_binary(t, key, strlen(key) + 1, data);
}

void jd_hash_remove(struct jd_hash_table *t, const char *key)
{
	jd_hash_remove_binary(t, key, strlen(key) + 1);
}

static struct jd_hash_node **_find_str_with_val(struct jd_hash_table *t,
...
{
	_free_nodes(t);
	memset(t->slots, 0, sizeof(struct jd_hash_node *) * t->num_slots);
	t->num_nodes = 0u;
}
...
static struct jd_hash_node *_next_slot(struct jd_hash_table *t, unsigned s)
{
	struct jd_hash_node *c = NULL;
	unsigned i;
